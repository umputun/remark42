...
func (s *RPC) addHandlers() {
	...
	s.Group("store", jrpc.HandlersGroup{
		...
		"get_user_details": s.getUserDetailsHndl,
		"delete":           s.deleteHndl,
		"close":            s.closeHndl,
	})
...
// getUserDetailsHndl get details for given request
func (s *RPC) getUserDetailsHndl(id uint64, params json.RawMessage) (rr jrpc.Response) {
	req := engine.UserDetailRequest{}
	if err := json.Unmarshal(params, &req); err != nil {
		return jrpc.Response{Error: err.Error()}
	}
	userDetails, err := s.eng.ListUserDetails(req)
	return jrpc.EncodeResponse(id, userDetails, err)
}
