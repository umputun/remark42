...
func (s *private) sendConfirmation(w http.ResponseWriter, r *http.Request) {
	...
	existingAddress, _ := s.dataService.GetStringUserDetail(store.Locator{SiteID: siteID}, user.ID, engine.UserEmail)
...
func (s *private) emailCtrl(w http.ResponseWriter, r *http.Request) {
	...
	val, err := s.dataService.SetStringUserDetail(locator, user.ID, engine.UserEmail, address)
	if err != nil {
