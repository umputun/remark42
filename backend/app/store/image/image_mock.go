...
// Save provides a mock function with given fields: fileName, userID, data
func (_m *MockStore) Save(fileName string, userID string, data []byte) (string, error) {
	ret := _m.Called(fileName, userID, data)

	var r0 string
	if rf, ok := ret.Get(0).(func(string, string, []byte) string); ok {
		r0 = rf(fileName, userID, data)
	} else {
		r0 = ret.Get(0).(string)
	}
