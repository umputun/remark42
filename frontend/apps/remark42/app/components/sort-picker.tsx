...
export function SortPicker() {
  ...
  const [items, itemsById] = useMemo(() => {
    const sortOptions = {
      ...(!StaticStore.config.hide_voting ? { '-score': intl.formatMessage(messages.best) } : {}),
      ...(!StaticStore.config.hide_voting ? { '+score': intl.formatMessage(messages.worst) } : {}),
      ...
      ...(!StaticStore.config.hide_voting ? { '-controversy': intl.formatMessage(messages.mostControversial) } : {}),
      ...(!StaticStore.config.hide_voting ? { '+controversy': intl.formatMessage(messages.leastControversial) } : {}),
    };
