...
describe.concurrent('Fetcher', () => {
	...
	it('sets sorted query string', async () => {
		vi.spyOn(window, 'fetch').mockResolvedValueOnce(new Response(''))
		await createFetcher('remark42', '/api').get('/query-string', { x: 3, p: 2, a: 1 })

		expect(window.fetch).toHaveBeenCalledWith('/api/query-string?a=1&p=2&x=3', expect.any(Object))
	})

	it.each(['get', 'post', 'put', 'delete'] as const)('implements %s', async (method) => {
		vi.spyOn(window, 'fetch').mockResolvedValueOnce(new Response(''))
		await createFetcher('remark42', '/api')[method]('/test')

		expect(window.fetch).toBeCalledWith(expect.any(String), expect.objectContaining({ method }))
	})
	...
	it('sends json', async () => {
		...
		const result = await createFetcher('remark42', '/api').post('/test/json', { payload })
	...
	it('sends text', async () => {
		...
		const result = await createFetcher('remark42', '/api').post('/test/text', { payload })
	...
	it.each([401, 403])('throws unauthorized on respose with status %s', async (status) => {
		vi.spyOn(window, 'fetch').mockResolvedValueOnce(new Response(null, { status }))
		const fetcher = createFetcher('remark42', '/api')
		await expect(fetcher.get('/test')).rejects.toThrowError('Unauthorized')
	})

	it.each([300, 400, 500])('throws error on respose with status %s', async (status) => {
		vi.spyOn(window, 'fetch').mockResolvedValueOnce(new Response('Error', { status }))
		const fetcher = createFetcher('remark42', '/api')
	...
	it('sets active token and then cleans it after unauthorized response', async () => {
		...
		vi.spyOn(window, 'fetch').mockResolvedValueOnce(new Response(undefined, { status: 401 }))
